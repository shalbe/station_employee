...
class debitOrder extends StatelessWidget {
  const debitOrder({super.key});
            ...
            body: Padding(
              ...
              child: Column(
                children: [
                  ...
                  cubit.debitOrder.isEmpty
                      ? Center(
                          child: defaultText(txt: 'لا يوجد طلبات بالدين'),
                        )
                      : ListView.separated(
                          padding: EdgeInsets.zero,
                          shrinkWrap: true,
                          itemBuilder: (context, index) => GetOrders(
                                data: cubit.debitOrder[index],
                              ),
                          separatorBuilder: (context, index) => SizedBox(
                                height: 15.h,
                              ),
                          itemCount: cubit.debitOrder.length),
                ],
              ),
            ),
          );
